...
int write(const void *buf, int buf_length)
{
    ...
    bool ret;
    ret = USART_SendBytes(UART4,(uint8_t*)buf,buf_length,1000);
	if(ret)
	{
		return buf_length;
	}
...
int read(uint8_t *buf, int buf_length, int timeout)
{
	...
	while(1)
	{
		...
		if((time_started + timeout) < GetTick())
		{
			//time out
			return i;
		}
...
//			timeinfo.tm_sec		= _buf.payload_rx_nav_pvt.sec;
		...
		_gps_position->timestamp = GetTick();
...
//			timeinfo.tm_sec		= _buf.payload_rx_nav_timeutc.sec;
	...
	case UBX_MSG_NAV_SVINFO:
		...
		_satellite_info->timestamp = GetTick();
...
int	waitForAck(const uint16_t msg, const unsigned timeout, uint8_t report)
{
	...
	gps_abstime time_started = GetTick();
	gps_abstime time_new =GetTick();
